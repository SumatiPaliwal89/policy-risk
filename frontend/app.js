...
async function submitPolicy() {
  ...
  btn.disabled = true;
  btn.innerHTML = `<div class="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div> Assessing…`;
  ...
  try {
    ...
    btn.disabled    = false;
    btn.innerHTML = `<svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z" clip-rule="evenodd"></path></svg> Assess Risk`;
  }
}
...
function renderSubmission(data) {
  ...
  banner.innerHTML = `
    <strong>Submission #${data.id}</strong> <span class="text-slate-300">|</span>
    <span class="state-chip ${stateClass[data.workflow_state]}">${data.workflow_state.replace('_', ' ')}</span>
    <span class="text-slate-300">|</span> ${data.policy_type.toUpperCase()} <span class="text-slate-300">|</span> ${data.state}
  `;
  ...
  document.getElementById('submitSummaryStats').innerHTML = `
    ...
    <div class="summary-box"><div class="s-val text-emerald-500">${s.Low||0}</div><div class="s-lbl">Low Risk</div></div>
    <div class="summary-box"><div class="s-val text-amber-500">${s.Medium||0}</div><div class="s-lbl">Medium Risk</div></div>
    <div class="summary-box flagged"><div class="s-val text-red-500">${s.High||0}</div><div class="s-lbl">High Risk</div></div>
    <div class="summary-box state-flags-box"><div class="s-val text-purple-600">${data.state_flag_count}</div><div class="s-lbl">State Flags</div></div>
  `;
  ...
  if (allStateFlags.length > 0) {
    ...
    alertDiv.innerHTML = `
      <strong class="flex items-center gap-2 mb-2"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg> State Regulation Issues (${data.state})</strong>
      ${unique.map(fid => {
...
function renderQueueTable(tbodyId, items, type) {
  ...
  tbody.innerHTML = items.map(item => {
    ...
    if (type === 'uw') {
      return `<tr>
        <td class="font-bold text-slate-700">#${item.id}</td>
        <td class="capitalize">${item.policy_type}</td>
        <td>${item.state}</td>
...
async function openReviewModal(submissionId, type) {
  ...
  // Use Tailwind display-flex class dynamically if we're not unhiding via CSS directly
  modal.classList.remove('hidden'); 
  document.getElementById('modalTitle').textContent    = `Reviewing Submission #${submissionId}`;
  document.getElementById('modalBody').innerHTML       = '<div class="flex flex-col items-center justify-center p-10"><div class="w-8 h-8 border-4 border-gray-200 border-t-themeBlue rounded-full animate-spin mb-4"></div><p class="text-slate-500 font-medium">Loading…</p></div>';
  ...
  try {
    ...
    document.getElementById('modalBody').innerHTML = `<p style="color:#ef4444" class="p-8 font-bold">Error: ${err.message}</p>`;
  }
}
...
function renderModalBody(data, type) {
  ...
  const clausesHtml = data.clauses.map(c => `
    <div class="clause-diff-item shadow-sm">
      <div class="clause-diff-header">
        <span>Clause ${c.index + 1}</span>
        <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
          ${(c.state_flags||[]).map(f =>
...
function renderModalFooter(data, type) {
  ...
  if (type === 'legal') {
    footer.innerHTML = `
      <input type="text" id="legalNote" class="form-input" style="margin-bottom:0;" placeholder="Legal review note (optional)" />
      <button class="btn-approve whitespace-nowrap" onclick="legalApprove(${data.id})">✓ Clear for UW</button>
      <button class="btn-reject" onclick="closeModal()">Cancel</button>
    `;
  } else {
    footer.innerHTML = `
      <input type="text" id="uwNote" class="form-input" style="margin-bottom:0;" placeholder="Underwriter note (optional)" />
      <button class="btn-approve whitespace-nowrap" onclick="modalFinalize(${data.id},'APPROVED')">✓ Approve</button>
  ...
  try {
    ...
    tbody.innerHTML = events.map(e => `
      <tr>
        ...
        <td><a href="#" class="text-themeBlue font-bold hover:underline" onclick="viewSubmission(${e.submission_id});return false">#${e.submission_id}</a></td>
...
function renderResult(data, originalClause) {
  ...
  document.getElementById('confBar').style.background =
    level === 'high' ? '#ef4444' : level === 'medium' ? '#e0d08b' : '#10b981';
  ...
  if (data.state_flags?.length > 0) {
    sfList.innerHTML = data.state_flags.map(f =>
      `<div class="flex items-start gap-2 mb-2">
        <span class="state-flag-chip severity-${f.severity?.toLowerCase()} shrink-0 mt-0.5">${f.flag_id}</span>
        <span class="text-xs text-slate-600 leading-relaxed">${f.description}</span>
      </div>`
    ).join('');
...
function renderBulkResults(data) {
  ...
  document.getElementById('bulkSummary').innerHTML = `
    ...
    <div class="summary-box"><div class="s-val text-emerald-500">${low}</div><div class="s-lbl">Low</div></div>
    <div class="summary-box"><div class="s-val text-amber-500">${medium}</div><div class="s-lbl">Medium</div></div>
    <div class="summary-box flagged"><div class="s-val text-red-500">${flagged}</div><div class="s-lbl">High</div></div>
  `;

  document.getElementById('resultsBody').innerHTML = data.map(r => `
    <tr>
      <td style="max-width:300px" class="truncate">${(r.clause||'').substring(0,100)}${(r.clause||'').length>100?'…':''}</td>
      <td><span class="badge ${r.risk_label?.toLowerCase()}">${r.risk_label}</span></td>
      <td class="font-mono text-sm">${r.confidence ? Math.round(r.confidence*100)+'%' : '—'}</td>
